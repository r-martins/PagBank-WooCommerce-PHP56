...
<fieldset name="PagSeguro">
    ...
    <div class="pslogo-container">
        ...
        wc_back_link( __( 'Voltar para Pagamentos', 'pagbank-connect' ), admin_url( 'admin.phppage=wc-settings&tab=checkout' ) );
        ?>
    </div>
    ...
    <nav class="nav-tab-wrapper ">
        <a href="<php echo admin_url( 'admin.phppage=wc-settings&tab=checkout&section=rm-pagbank' ) ?>#tab-general" class="nav-tab <php echo $this->id === 'rm-pagbank' ? 'nav-tab-active' : '' ?>"><php esc_html_e('Geral', 'pagbank-connect') ?></a>
        <a href="<php echo admin_url( 'admin.phppage=wc-settings&tab=checkout&section=rm-pagbank-cc' ) ?>#tab-credit-card" class="nav-tab <php echo $this->id === 'rm-pagbank-cc' ? 'nav-tab-active' : '' ?>"><php esc_html_e('Cartão de Crédito', 'pagbank-connect') ?></a>
        <a href="<php echo admin_url( 'admin.phppage=wc-settings&tab=checkout&section=rm-pagbank-pix' ) ?>#tab-pix" class="nav-tab <php echo $this->id === 'rm-pagbank-pix' ? 'nav-tab-active' : '' ?>"><php esc_html_e('PIX', 'pagbank-connect') ?></a>
        <a href="<php echo admin_url( 'admin.phppage=wc-settings&tab=checkout&section=rm-pagbank-boleto' ) ?>#tab-boleto" class="nav-tab <php echo $this->id === 'rm-pagbank-boleto' ? 'nav-tab-active' : '' ?>"><php esc_html_e('Boleto', 'pagbank-connect') ?></a>
        <a href="<php echo admin_url( 'admin.phppage=wc-settings&tab=checkout&section=rm-pagbank-redirect' ) ?>#tab-redirect" class="nav-tab <php echo $this->id === 'rm-pagbank-redirect' ? 'nav-tab-active' : '' ?>"><php esc_html_e('Checkout PagBank', 'pagbank-connect') ?></a>
        <a href="<php echo admin_url( 'admin.phppage=wc-settings&tab=checkout&section=rm-pagbank-recurring-settings' ) ?>#tab-recurring" class="nav-tab <php echo $this->id === 'rm-pagbank-recurring' ? 'nav-tab-active' : '' ?>"><php esc_html_e('Recorrência', 'pagbank-connect') ?></a>
    </nav>
    <php if ($this->id === 'rm-pagbank'): ?>
        ...
        <a href="<php echo admin_url( 'admin.phppage=wc-settings&tab=shipping&section=rm_enviofacil' ) ?>" class="button button-secondary" title="<php esc_html_e('Economize no Frete com Envio Fácil', 'pagbank-connect');?>"><php esc_html_e('📦 Envio Fácil', 'pagbank-connect') ?></a>
    <php endif; ?>
    ...
    <script type="text/javascript">
        ...
        window.addEventListener('message', (event) => {
            ...
            if (event.data && event.data.connectKey && event.origin.indexOf('pbintegracoes.com') !== -1) {
                const connectKeyField = document.querySelector('input array(name="woocommerce_rm-pagbank_connect_key")');
                if (connectKeyField) {
                    ...
                    if(confirm('Já preenchemos a Connect Key pra você. Deseja salvar agora?')){
                        document.querySelector('button array(name="save")').click();
                    }
                }
            }
        });
