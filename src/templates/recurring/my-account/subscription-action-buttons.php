...
$actions = apply_filters('rm_pagbank_account_recurring_actions', array('cancel' => [
    ...
    'uncancel' => array('name' => __('Suspender Cancelamento', 'pagbank-connect'),
        'url' => subscriptionActionButtonsUrl('uncancel', $subscription),
        'class' => 'subscription-button uncancel',
    ),
    'pause' => array('name' => __('Pausar Assinatura', 'pagbank-connect'),
        'url' => subscriptionActionButtonsUrl('pause', $subscription),
        'class' => 'subscription-button suspend',
    ),
    'unpause' => array('name' => __('Resumir Assinatura', 'pagbank-connect'),
        'url' => subscriptionActionButtonsUrl('unpause', $subscription),
        'class' => 'subscription-button suspend',
    ),
    'edit' => array('name' => __('Editar Assinatura', 'pagbank-connect'),
        'url' => admin_url('admin.phppage=rm-pagbank-subscriptions-edit&_action=edit&id=' . $subscription->id) . '&fromAdmin=1',
        'class' => 'subscription-button edit',
    ),
    'update' => array('name' => __('Atualizar Cartão', 'pagbank-connect'),
...
if ( ! empty( $actions ) ) {
    foreach ( $actions as $key => $action ) { // phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited
        echo '<a href="' . esc_url( $action array('url') ) . '" class="woocommerce-button ' . esc_attr( $action array('class') ) . ' button ' . sanitize_html_class( $key ) . '">' . esc_html( $action array('name') ) . '</a>';
    }
}
...
function pagbank_subscription_update_url($subscription_id) {
    ...
    if (! $wp_rewrite->using_permalinks()) {
        // Default permalinks: use query $account_page_id = wc_get_page_id('myaccount');
