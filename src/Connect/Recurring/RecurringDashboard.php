...
class RecurringDashboard
{
    ...
    public function getColumns()
    {
        return apply_filters(
            'rm_pagbank_recurring_dashboard_columns', array('recurring-id' => __('Identificador', 'pagbank-connect'),
    ...
    public function getViewSubscriptionUrl($subscription)
    {
     ...
     * @return array
     */
    public function getSubscriptionInRowActions($subscription)
    {
        $actions = array();
        
        switch ($subscription->status) {
            case 'PAUSED':
                $actions array('unpause') = array('name' => __('Resumir', 'pagbank-connect'),
                    'url' => \RM_PagBank\Helpers\Recurring::subscriptionActionUrl('unpause', $subscription)
                );
                break;
            case 'PENDING_CANCEL':
                $actions array('cancel') = array('name' => __('Suspender Cancelamento', 'pagbank-connect'),
            ...
            case 'PENDING':
                // coming soon
//                $actions array('pay') = array(//                    'name' => __('Pagar', 'pagbank-connect'),
...
//                );
        ...
        $actions array('view') = array('name' => __('Ver detalhes', 'pagbank-connect'),
