...
// use RM_PagBank\EnvioFacil; // PHP 5.6 compatibility
...
add_filter('woocommerce_get_settings_checkout' , array(Connect\Recurring::class, 'recurringSettingsFields') , 10, 2 );
add_filter('woocommerce_settings_checkout' , array(Connect\Recurring::class, 'recurringHeaderSettingsSection') , 10, 2 );

//envio facil
add_filter('woocommerce_shipping_methods', array(EnvioFacil::class, 'addMethod'));

//recurring and styles
add_filter('woocommerce_enqueue_styles', array(Gateway::class, 'addStyles'), 99999, 1);
add_filter('woocommerce_enqueue_styles', array(Gateway::class, 'addStylesWoo'), 99999, 1);

//not needed so far...
register_activation_hook(__FILE__, array(Connect::class, 'activate'));
register_deactivation_hook(__FILE__, array(Connect::class, 'deactivate'));
register_uninstall_hook(__FILE__, array(Connect::class, 'uninstall'));

// Upgrading scripts
add_action('plugins_loaded', array(Connect::class, 'upgrade'));